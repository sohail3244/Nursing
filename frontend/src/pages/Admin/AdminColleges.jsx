...
import { useColleges, useDeleteCollege, useUpdateCollege, useAddCollege } from '../../hooks/useCollege';
...
function AdminColleges() {
  ...
  const handleModalSubmit = (formDataToSend) => {
    if (editingCollege) {
      updateMutation.mutate({ id: editingCollege.id, data: formDataToSend }, {
        onSuccess: () => {
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center p-20 gap-3">
            ...
            <span className="text-gray-400 font-medium">Loading colleges...</span>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead className="bg-gray-50/50 border-b border-gray-100">
                <tr>
                  <th className="px-6 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">College Info</th>
                  ...
                  <tr key={college.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex justify-end gap-2 ">
                        ...
                        <button onClick={() => handleDelete(college.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
                          <Trash2 size={18} />
                        </button>
                      </div>
                    </td>
